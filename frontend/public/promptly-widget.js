...
(function () {
  ...
  class PromptlyWidget {
    ...
    async fetchChatbotConfig() {
      try {
        const response = await fetch(
          `${API_BASE}/chat/config/${this.chatbotId}`
        );
    ...
    createWidget() {
      ...
      container.innerHTML = `
        <button class="promptly-chat-button" id="promptly-toggle-btn" style="background: ${
          this.chatbot?.primaryColor || "#3B82F6"
        }">
        ...
        <div class="promptly-chat-window" id="promptly-chat-window">
          <div class="promptly-chat-header" style="background: ${
            this.chatbot?.primaryColor || "#3B82F6"
          }">
          ...
          <div class="promptly-chat-input-container">
            ...
            <button class="promptly-chat-send" id="promptly-send-btn" style="background: ${
              this.chatbot?.primaryColor || "#3B82F6"
            }">Send</button>
          </div>
        </div>
      ...
      // Add event listeners
      document
        .getElementById("promptly-toggle-btn")
        .addEventListener("click", () => this.toggle());
      document
        .getElementById("promptly-close-btn")
        .addEventListener("click", () => this.close());
      document
        .getElementById("promptly-send-btn")
        .addEventListener("click", () => this.sendMessage());
      document
        .getElementById("promptly-input")
        ...
        .map((msg, i) => {
          ...
          `;
            ...
            <div class="promptly-message bot">
              <div class="promptly-message-bubble" style="background: #374151; color: #F3F4F6; border: 1px solid ${
                this.chatbot?.primaryColor
              }40;">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                  <div style="width: 24px; height: 24px; border-radius: 50%; background: ${
                    this.chatbot?.primaryColor || "#3B82F6"
                  }; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: bold; color: white; overflow: hidden;">
                    ${avatarContent}
                  </div>
      ...
      for (let line of lines) {
        ...
        if (line.trim().startsWith("```")) {
          if (!inCode) {
            ...
            inCode = true;
            out +=
              '<pre style="background:#1F2937;border:1px solid #374151;border-radius:8px;padding:10px;overflow:auto"><code>';
          } else {
            inCode = false;
            out += "</code></pre>";
          }
        ...
        if (/^###\s+/.test(line)) {
          closeLists();
          out += `<h3 style="font-weight:700;margin:8px 0 6px 0">${line.replace(
            /^###\s+/,
            ""
          )}</h3>`;
          continue;
        ...
        if (/^##\s+/.test(line)) {
          closeLists();
          out += `<h2 style="font-weight:700;margin:10px 0 6px 0">${line.replace(
            /^##\s+/,
            ""
          )}</h2>`;
          continue;
        ...
        if (/^#\s+/.test(line)) {
          closeLists();
          out += `<h1 style="font-weight:700;margin:12px 0 6px 0">${line.replace(
            /^#\s+/,
            ""
          )}</h1>`;
          continue;
        ...
        if (/^\s*\d+\.\s+/.test(line)) {
          if (!inOl) {
            closeLists();
            inOl = true;
            out += '<ol style="padding-left:18px;margin:6px 0">';
          }
        ...
        line = line
          ...
          .replace(
            /`([^`]+)`/g,
            '<code style="background:#1F2937;padding:2px 6px;border-radius:4px">$1</code>'
          );
